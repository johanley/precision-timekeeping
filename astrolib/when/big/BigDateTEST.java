...
public class BigDateTEST {
  ...
  @Test public void unequal() {
    BigDate x = BigDate.gregorian(2025, 10, 2);
    BigDate y = BigDate.julian(2025, 10, 2);
    assertFalse(x.equals(y));
    
    x = BigDate.gregorian(2025, 10, 2);
    y = BigDate.gregorian(2025, 10, 3);
    assertFalse(x.equals(y));
  }
  ...
  @Test public void plusOrMinus() {
    Stream.of(BigCalendar.values()).forEach(calendar -> plusMinusSanity(calendar));
    nextPrevTest();
  }
  
  private void sanityTest(int y, int m, int d) {
    for (BigCalendar calendar : BigCalendar.values()) {
      BigDate dt = BigDate.from(y, m, d, calendar);
      ...
      //neg year
      dt = BigDate.from(-y, m, d, calendar);
      assertEquals(-y, dt.year());
      assertEquals(m, dt.month());
      assertEquals(d, dt.day());
      assertEquals(calendar, dt.calendar());
  ...
  private void testDayOfYearNonLeap(int notLeapYear, BigCalendar calendar) {
    BigDate x = BigDate.from(notLeapYear, 1, 1, calendar);
    ...
    BigDate y = BigDate.from(notLeapYear, 12, 31, calendar);
    assertEquals(365, y.dayOfYear());
  }
  
  private void testDayOfYearLeap(int leapYear, BigCalendar calendar) {
    BigDate x = BigDate.from(leapYear, 1, 1, calendar);
  ...
  private void plusMinusSanity(BigCalendar calendar) {
    //near a leap day, and across month-end
    BigDate x = BigDate.from(1960, 2, 27, calendar);
    BigDate y = x.plusOrMinusDays(1);
    ...
    x = BigDate.from(1960, 3, 1, calendar);
    y = x.plusOrMinusDays(-1);
    assertEquals(29, y.day());
    assertEquals(2, y.month());
    assertEquals(1960, y.year());
    ...
    //across a year-end
    x = BigDate.from(1960, 12, 31, calendar);
    y = x.plusOrMinusDays(1);
    assertEquals(1, y.day());
    assertEquals(1, y.month());
    assertEquals(1961, y.year());
    
    x = BigDate.from(1961, 1, 1, calendar);
    y = x.plusOrMinusDays(-1);
  ...
  /** d is mid-month-ish. */
  private void nextPrevSanityTest(int y, int m, int d, BigCalendar calendar) {
    BigDate a = BigDate.from(y, m, d, calendar);
    BigDate aPlus1 = BigDate.from(y, m, d+1, calendar); 
    BigDate aMinus1 = BigDate.from(y, m, d-1, calendar); 
    assertTrue(a.next().eq(aPlus1));
    assertTrue(a.previous().eq(aMinus1));
  }
  
  /** The date is Feb 28.  */
  private void nextPrevFeb28Test(int y, BigCalendar calendar) {
    boolean isLeap = calendar.isLeap(y);
    BigDate a = BigDate.from(y, 2, 28, calendar);
    BigDate aPlus1 = isLeap ? BigDate.from(y, 2, 29, calendar) : BigDate.from(y, 3, 1, calendar); 
    BigDate aMinus1 = BigDate.from(y, 2, 27, calendar); 
    assertTrue(a.next().eq(aPlus1));
    assertTrue(a.previous().eq(aMinus1));
  }

  /** The date is the end of the month. The month is not February, and not December. */
  private void nextPrevMonthEndTest(int y, int m, BigCalendar calendar) {
    assertFalse(m == 2 || m == 12);
    boolean isLeap = calendar.isLeap(y);
    int lastDay = Month.of(m).length(isLeap);
    BigDate a = BigDate.from(y, m, lastDay, calendar);
    BigDate aPlus1 = BigDate.from(y, m + 1, 1, calendar); 
    BigDate aMinus1 = BigDate.from(y, m, lastDay - 1, calendar); 
    assertTrue(a.next().eq(aPlus1));
    assertTrue(a.previous().eq(aMinus1));
  }

  /** The month is December. */
  private void nextPrevYearEndTest(int y, BigCalendar calendar) {
    BigDate a = BigDate.from(y, 12, 31, calendar);
    BigDate aPlus1 = BigDate.from(y + 1, 1, 1, calendar); 
    BigDate aMinus1 = BigDate.from(y, 12, 30, calendar); 
    assertTrue(a.next().eq(aPlus1));
    assertTrue(a.previous().eq(aMinus1));
  }
  ...
  private void orderingTest(int y, int m, int d) {
    for(BigCalendar calendar: BigCalendar.values()) {
      BigDate p = BigDate.from(y, m, d, calendar);
      
      BigDate q = p.next();
      assertTrue(p.lt(q));
      assertTrue(q.gt(p));
      
      BigDate o = p.previous();
      assertTrue(p.gt(o));
      assertTrue(o.lt(p));
      ...
      BigDate q2 = BigDate.from(q.year(), q.month(), q.day(), q.calendar());
  ...
  private void startOfMonthTest(int y, int m, int d) {
    for(BigCalendar calendar : BigCalendar.values()) {
      BigDate a = BigDate.from(y, m, d, calendar);
      BigDate b = BigDate.from(y, m, 1, calendar);
      assertTrue(b.eq(a.startOfMonth()));
      
      //neg year
      a = BigDate.from(-y, m, d, calendar);
      b = BigDate.from(-y, m, 1, calendar);
      assertTrue(b.eq(a.startOfMonth()));
    }
  }
  ...
  private void lastOfMonthTest(int y, int m, int d) {
    for(BigCalendar calendar : BigCalendar.values()) {
      ...
      BigDate b = BigDate.from(y, m, endOfMonth, calendar);
      assertTrue(b.eq(a.endOfMonth()));
      
      //neg year
      a = BigDate.from(-y, m, d, calendar);
      endOfMonth = Month.of(m).length(calendar.isLeap(-y));
      b = BigDate.from(-y, m, endOfMonth, calendar);
      assertTrue(b.eq(a.endOfMonth()));
    }
  }
}
