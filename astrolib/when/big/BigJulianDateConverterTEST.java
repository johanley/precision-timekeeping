...
public class BigJulianDateConverterTEST {
  ...
  private void testYear(long year, double jan_0_in_given_year, BigCalendar calendar) {
    testDateToJdForYear(year, jan_0_in_given_year, calendar);
    testJdToDateForYear(year, jan_0_in_given_year, calendar);
  }
  
  private void testDate(long year, int month, double day, double jd, BigCalendar calendar) {
    testDateToJd(year, month, day, jd, calendar);
    testJdToDate(year,  month,  day, jd, calendar);
  }

  private void testDateToJdForYear(long year, double jan_0_given_year, BigCalendar calendar) {
  ...
  private void testJdToDateForYear(long year, double jan_0_in_given_year, BigCalendar calendar) {
  ...
  private void testDateToJd(long year, int month, double day, double jd_expected, BigCalendar calendar) {
    BigJulianDateConverter convert = BigJulianDateConverter.using(calendar);
    BigJulianDate jd = convert.toJulianDate(year, month, BigDecimal.valueOf(day), TIMESCALE);
    assertTrue(BigDecimal.valueOf(jd_expected).compareTo(jd.jd()) == 0);
  }
  
  private void testJdToDate(long y_expected, int m_expected, double d_expected, double jd, BigCalendar calendar) {
    BigJulianDateConverter convert = BigJulianDateConverter.using(calendar);
    BigDateTime dt = convert.toDateTime(BigJulianDate.from(BigDecimal.valueOf(jd), TIMESCALE));
