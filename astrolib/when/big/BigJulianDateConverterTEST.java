...
import static astrolib.when.big.BigCalendar.*;
...
/** JUnit 4 tests.*/
public class BigJulianDateConverterTEST {
  ...
  @Test public void specificCasesJulian() {
    BigCalendar cal = BigCalendar.JULIAN;
  ...
  @Test public void specificCasesGregorian() {
    BigCalendar cal = BigCalendar.GREGORIAN;
  ...
  private static final BigTimescaleCommon TIMESCALE = BigTimescaleCommon.TT;
  
  // It's easy to compute the JD manually.
  private void testSmallYears(BigCalendar calendar) {
    BigDecimal base = calendar.julianDateJan0Year0();
  ...
  private void testYear(int year, double jan_0_in_given_year, BigCalendar calendar) {
    testDateToJdForYear(year, jan_0_in_given_year, calendar);
    testJdToDateForYear(year, jan_0_in_given_year, calendar);
  }
  
  private void testDate(int year, int month, double day, double jd, BigCalendar calendar) {
    testDateToJd(year, month, day, jd, calendar);
    testJdToDate(year,  month,  day, jd, calendar);
  }

  private void testDateToJdForYear(int year, double jan_0_given_year, BigCalendar calendar) {
    BigDate day = BigDate.from(year, 1,  1, calendar);
  ...
  private void testJdToDateForYear(int year, double jan_0_in_given_year, BigCalendar calendar) {
    BigDate day = BigDate.from(year, 1, 1, calendar); //Jan 1 of the given year
  ...
  private void testDateToJd(int year, int month, double day, double jd_expected, BigCalendar calendar) {
    BigJulianDateConverter convert = BigJulianDateConverter.using(calendar);
    BigJulianDate jd = convert.toJulianDate(year, month, day, TIMESCALE);
    assertEquals(jd_expected, jd.jd(), Consts.EPSILON);
  }
  
  private void testJdToDate(int y_expected, int m_expected, double d_expected, double jd, BigCalendar calendar) {
    BigJulianDateConverter convert = BigJulianDateConverter.using(calendar);
    BigDateTime dt = convert.toDateTime(BigJulianDate.from(jd, TIMESCALE));
